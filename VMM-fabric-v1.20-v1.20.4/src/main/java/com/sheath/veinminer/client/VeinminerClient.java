...
public final class VeinminerClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        ClientPlayConnectionEvents.JOIN.register((handler, sender, client) ->
                ClientPlayNetworking.send(HandshakeC2SPayload.ID, HandshakeC2SPayload.empty()));
        ...
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            ...
            if (pressed != lastState) {
                lastState = pressed;
                ClientPlayNetworking.send(KeyStateC2SPayload.ID, KeyStateC2SPayload.buf(pressed));
            }
        });
    }
}
