...
public final class VeinMinerController {
    ...
    public boolean handleBlockBreak(ServerWorld world,
        ...
        if (!player.canHarvest(state) && state.isToolRequired() && !allowsIncorrectTool(state)) {
            return true;
        }
        ...
        int remaining = tool.isDamageable() ? tool.getMaxDamage() - tool.getDamage() : Integer.MAX_VALUE;
        if (snapshot.general().checkToolDurability() && tool.isDamageable() && remaining <= reserve) {
    ...
    private int computeBlockCap(ItemStack tool, int remaining, int reserve) {
        ...
        if (!snapshot.general().checkToolDurability() || !tool.isDamageable()) {
            return baseLimit;
        }
    ...
    private VeinPlan findVein(ServerWorld world,
                              BlockPos origin,
    ...
    private void applyPlan(ServerWorld world,
        ...
        boolean checkDurability = snapshot.general().checkToolDurability() && actualTool.isDamageable();
        int remaining = actualTool.isDamageable() ? actualTool.getMaxDamage() - actualTool.getDamage() : Integer.MAX_VALUE;
