...
public final class GeneralConfig extends TomlConfigFile {
    ...
    public void load() {
        try (CommentedFileConfig config = loadConfig()) {
            ...
            particles.durationTicks = clampParticleDuration(config.getOrElse("visual.particle_duration_ticks",
    ...
    private void writeBack(CommentedFileConfig config) {
        ...
        config.setComment("visual.particle_duration_ticks",
                "Particle outline duration in ticks (1-1200; 20 ticks = 1 second).");
    ...
    private int clampParticleDuration(int value) {
        int clamped = Math.max(1, Math.min(ParticleSettings.MAX_DURATION_TICKS, value));
        if (clamped != value) {
            Log.warn("visual.particle_duration_ticks={} is outside the 1-{} range. Clamping to {}",
                    value, ParticleSettings.MAX_DURATION_TICKS, clamped);
        }
        return clamped;
    }
    ...
    public static final class ParticleSettings {
        ...
        public void setDurationTicks(int durationTicks) {
            this.durationTicks = Math.max(1, Math.min(MAX_DURATION_TICKS, durationTicks));
        }
