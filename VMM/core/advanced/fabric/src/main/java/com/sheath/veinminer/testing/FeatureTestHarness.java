...
public final class FeatureTestHarness {
    ...
    private static final Method CREATE_PLAN = lookupCreatePlan();
     ...
     * Begins a feature test run. Returns {@code false} when another run is already in progress.

     */

    public boolean start(ServerCommandSource source) {
        ...
        try {
            ...
            if (!(source.getEntity() instanceof ServerPlayerEntity playerEntity)) {

                source.sendError(Translations.translate("command.veinminer.test.player_only"));

                Log.warn("Feature test aborted: non-player source '{}'", source.getName());
            ...
            source.sendFeedback(() -> Translations.translate("command.veinminer.test.starting"), false);

            source.sendFeedback(() -> Translations.translate("command.veinminer.test.running"), false);
            ...
            source.sendFeedback(() -> Translations.translate("command.veinminer.test.summary", passed, steps.size()), false);
            ...
            for (TestResult result : results) {
                ...
                if (result.success()) {
                    ...
                    source.sendFeedback(() -> Translations.translate("command.veinminer.test.step.pass", result.name()), false);

                } else {

                    source.sendError(Translations.translate("command.veinminer.test.step.fail", result.name(), result.message()));
    ...
    private List<TestStep> createSteps() {
        ...
        steps.add(new TestStep("Particle Toggle", ctx -> {
            ...
            ctx.expectCommandSuccess("veinminer particles toggle");
    ...
    private void runWorldSimulation(TestContext ctx) throws Exception {
        ...
        BlockPos origin = ctx.player().getBlockPos().up();
        ...
        boolean originalSneaking = ctx.player().isSneaking();

        ItemStack originalMainHand = ctx.player().getMainHandStack().copy();

        ItemStack tool = new ItemStack(Items.IRON_PICKAXE);

        ctx.player().setStackInHand(Hand.MAIN_HAND, tool);

        ctx.player().setSneaking(snapshot.general().requireCrouch());
        ...
        try {
            ...
            if (FIND_VEIN.getParameterCount() == 4) {

                veinResult = FIND_VEIN.invoke(controller, world, origin, originState, limit);

            } else {

                veinResult = FIND_VEIN.invoke(controller, world, origin, originState, limit, 0, tool.copy());

            }

            Object plan = veinResult;

            if (!APPLY_PLAN.getParameterTypes()[3].isInstance(plan)) {

                if (CREATE_PLAN == null) {

                    throw new IllegalStateException("Unable to access VeinMinerController#createPlan");

                }

                plan = CREATE_PLAN.invoke(controller, origin, originState, veinResult, 0, tool.copy());

            }

            APPLY_PLAN.invoke(controller, world, ctx.player(), tool, plan, reserve);

            for (BlockPos pos : cluster) {

                ctx.assertTrue(world.getBlockState(pos).isAir(), "Expected block at " + pos + " to be mined");

            }

            clearEntities(world, cluster);

        } finally {

            ctx.player().setStackInHand(Hand.MAIN_HAND, originalMainHand);

            ctx.player().setSneaking(originalSneaking);
    ...
    private void clearEntities(ServerWorld world, List<BlockPos> cluster) {

        BlockPos origin = cluster.get(0);

        Box area = new Box(origin).expand(2.0);
    ...
    private static Method lookupFindVein() {
        ...
        try {
            ...
            try {
                ...
                throw new IllegalStateException("Unable to access VeinMinerController#findVein", legacySignatureException);
    ...
    private static Method lookupCreatePlan() {
    ...
    private static Method lookupApplyPlan() {
        ...
        Class<?> primaryPlanType = CREATE_PLAN != null ? CREATE_PLAN.getReturnType() : FIND_VEIN.getReturnType();
        ...
        try {
            ...
            Class<?> fallbackPlanType = FIND_VEIN.getReturnType();

            if (primaryPlanType == fallbackPlanType) {

                throw new IllegalStateException("Unable to access VeinMinerController#applyPlan", primaryException);
            ...
            try {
                ...
                throw new IllegalStateException("Unable to access VeinMinerController#applyPlan", fallbackException);
    ...
    private static final class TestContext {
        ...
        TestContext(Bootstrap bootstrap, ServerCommandSource source, ServerPlayerEntity player, PlayerSettingsStore store) {
            ...
            this.dispatcher = source.getServer().getCommandManager().getDispatcher();
    ...
    private static final class PlayerSnapshot {
        ...
        static PlayerSnapshot capture(PlayerSettingsStore store, ServerPlayerEntity player) {
        ...
        void restore(PlayerSettingsStore store, ServerPlayerEntity player) {
